...
/* MANUAL ORDER OR RANDOM ORDER */
   ...
   if(argc > 1){ // If there is input manually from user.
      ...
      if( atoi(argv[1]) > 0 && atoi(argv[1]) < 5 && \
          ...
          atoi(argv[3]) >= 0 && atoi(argv[3]) < 10 && argc < 5 ){
        ...
        printf("\033[45m##********************* < HELPING MENU > *********************##\033[0m\n\n");
        ...
        printf("> You can choose between zones [ 1 | 2 | 3 | 4 ]. (Then press ENTER: )\nPlease choose the # of zone: ");
        scanf("%c", &msg_c[1]);
        getchar(); // Takes the character putted by ENTER key.
      ...
      /************************* RANDOM ORDER *************************/   
        ...
        // Generate random # of tickets [1-4].
        msg_c[0] = rand()%(53 - 49) + 49; 
        
        // Generate random # of zone [0->A (10%). 1, 
        // 2->B(20%). 3, 4, 5->C (30%). 6, 7, 8, 9->D (40%)].
        msg_c[1] = rand()%(58 - 48) + 48; 
        ...
        // Generate random credit card number [0-9]. 0 is the fail number (10%).
        msg_c[2] = rand()%(58 - 48) + 48; 
     ...
     /* Print a message for each of the 4 cases  */
      ...
      switch (line) {
        ...
        default: // // Case: (68 + id), we pass the id in "coded form". Decoding it using (-68) in printing below
          ...
          printf("\n\n\n\033[32m-> Your order has been completed successfully."
                 "\n   The ID of reservation is #%d, you have %c seats in zone %c."
                 "\n   The amount of transaction is %d€.\033[0m\n\n\n", (line - 68), msg_c[0], msg_c[1] + 16, calc_ammount(msg_c[0] - 48, msg_c[1] - 48) );
          fflush(stdout);  // Flush stdout to output     
      }
